  ...
  const deleteTag = async () => {
    try {
      ...
      const response = await toast.promise(request, {
        pending: "Deleting Analytics",
        success: `Analytics Deleted Successfully!`,
...
const AnalyticsList = () => {
  ...
  const loadData = async (cancelToken) => {
    try {
      const response = await axios.get("/admin/analytics/list", {
        cancelToken: cancelToken.token,
        headers: { authorization: "Bearer " + auth.token },
      });
      console.log(response?.data);
      response?.data?.success && setData(response.data.analytics);
    } catch (err) {
      console.error(err);
    }
  };
